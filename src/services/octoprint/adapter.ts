...
export class Adapter<T = IdType> {
  ...
  async connect(id: T, loginDto: LoginDto): Promise<void> {
    ...
    const websocketUrl = urlToWs(loginDto.printerURL);

    await this.open(websocketUrl);
  }

  private async open(url: string | URL): Promise<void> {
    if (this.socket) {
  ...
  private async reconnectWebSocketWithInterval(url: string): Promise<void> {
    ...
    this.reconnectInterval = setInterval(async () => {
      ...
      try {
        ...
        console.error("Reconnection failed, retrying in 15 seconds", err);
      }
    }, 15000);
  }

  private async openWebSocketWithTimeout(url: string | URL, timeout: number): Promise<WebSocket> {
    if (this.socket) {
      throw new Error("Socket already exists, ignoring open request");
    }
  ...
  protected async onError(error: WsEvent): Promise<void> {
    console.error("Socket error, initiating reconnect:", error);
    ...
    await this.reconnectWebSocketWithInterval(this.socket!.url);
  }

  protected async onClose(closeEvent: CloseEvent): Promise<void> {
    console.error("Socket closed, initiating reconnect:", closeEvent);
  ...
  protected async onMessage(data: Data): Promise<void> {
    console.error("Subclass socket message received");
  }
